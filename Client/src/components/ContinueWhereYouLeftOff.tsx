...
export default function ContinueWhereYouLeftOff() {
  ...
  return (
    <Container>
      <div className="grid p-2 sm:p-4 bg-[#B2CCFF82] grid-cols-1 xl:grid-cols-[auto_463px] gap-4 sm:gap-6 items-start">
        ...
        <div className="w-full">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 sm:mb-6 gap-3">
            ...
            <button className="px-5 mr-12 py-2 bg-blue-600 text-white text-sm font-semibold rounded-full hover:bg-blue-700 transition-colors shadow-md whitespace-nowrap">
              View More
            </button>
          </div>
          ...
          <div className="flex flex-wrap gap-4 justify-center sm:justify-start">
            {leftItems.map((item) => (
              <div
                key={item.id}
                className="w-[220px] h-[249px] bg-white rounded-[21px] shadow-lg hover:shadow-xl transition-shadow p-3"
              >
                {/* Thumbnail */}
                <div className="relative w-[196px] h-[127px] rounded-xl overflow-hidden mb-3">
                  <Image
                    src={item.image}
                    alt={item.title}
                    width={400}
                    ...
                    priority={item.id === 1} // Optional: prioritize first image
                  />
                  {item.hasVideo && (
                    ...
                    <div className="flex items-center gap-1">
                      <div
                        className={`w-1 h-1 rounded-full ${
                          item.progress > 0 ? 'bg-blue-600' : 'bg-orange-500'
                        }`}
                      />
        ...
        <div className="w-full xl:w-[463px]">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 sm:mb-6 gap-3">
            ...
            <div className="flex items-center mr-12 gap-2">
              ...
              <button className="px-5 py-2 bg-blue-600 text-white text-sm font-semibold rounded-full hover:bg-blue-700 transition-colors shadow-md whitespace-nowrap">
                View More
              </button>
            </div>
          </div>

          <div className="w-full min-h-[345px] bg-gradient-to-b from-blue-50/50 to-blue-50/20 rounded-[24px] p-3 sm:p-4 space-y-3">
